...
link_toolbox = load("bin/link-toolbox.py")
...
def test_entries_land_at_the_same_relative_path(checker, tmp_path):
    ...
    code, _ = checker(link_toolbox, argv(root, target, "go", "--yes"), tmp_path)
    assert code == 0
...
def test_includes_are_followed(checker, tmp_path):
    ...
    root, target = toolbox(tmp_path), project(tmp_path)
    checker(link_toolbox, argv(root, target, "go", "--yes"), tmp_path)
...
def test_a_set_without_includes_stands_alone(checker, tmp_path):
    ...
    root, target = toolbox(tmp_path), project(tmp_path)
    checker(link_toolbox, argv(root, target, "lone", "--yes"), tmp_path)
...
def test_absolute_is_available_for_a_toolbox_that_does_not_travel(checker, tmp_path):
    ...
    root, target = toolbox(tmp_path), project(tmp_path)
    checker(link_toolbox, argv(root, target, "lone", "--absolute", "--yes"), tmp_path)
...
def test_a_real_file_is_never_overwritten(checker, tmp_path):
    ...
    code, out = checker(link_toolbox, argv(root, target, "lone", "--yes"), tmp_path)
    assert code == 1
...
def test_a_manifest_naming_an_absent_file_links_nothing(checker, tmp_path):
    ...
    code, out = checker(link_toolbox, argv(root, target, "go", "--yes"), tmp_path)
    assert code == 1
...
def test_an_unknown_set_names_the_ones_that_exist(checker, tmp_path):
    ...
    root, target = toolbox(tmp_path), project(tmp_path)
    code, out = checker(link_toolbox, argv(root, target, "rust", "--yes"), tmp_path)
    assert code == 1
...
def test_sets_that_include_each_other_are_refused(checker, tmp_path):
    ...
    code, out = checker(link_toolbox, argv(root, project(tmp_path), "a"), tmp_path)
    assert code == 1
    assert "cycle" in out
...
def test_no_terminal_and_no_yes_writes_nothing(checker, tmp_path):
    ...
    root, target = toolbox(tmp_path), project(tmp_path)
    code, out = checker(link_toolbox, argv(root, target, "lone"), tmp_path)
    assert code == 1
    assert "--yes" in out
...
def test_check_fails_on_a_missing_link_and_writes_nothing(checker, tmp_path):
    ...
    root, target = toolbox(tmp_path), project(tmp_path)
    code, _ = checker(link_toolbox, argv(root, target, "go", "--check"), tmp_path)
    assert code == 1
    assert not any(target.iterdir())
...
def test_check_passes_once_the_links_are_there(checker, tmp_path):
    ...
    root, target = toolbox(tmp_path), project(tmp_path)
    checker(link_toolbox, argv(root, target, "go", "--yes"), tmp_path)
    code, out = checker(link_toolbox, argv(root, target, "go", "--check"), tmp_path)
    assert code == 0
    assert "present and correct" in out
...
def test_a_link_pointing_at_the_wrong_file_is_repaired(checker, tmp_path):
    ...
    code, _ = checker(link_toolbox, argv(root, target, "lone", "--yes"), tmp_path)
    assert code == 0
...
def test_a_link_left_behind_by_a_dropped_set_is_found(checker, tmp_path):
    ...
    root, target = toolbox(tmp_path), project(tmp_path)
    checker(link_toolbox, argv(root, target, "go", "--yes"), tmp_path)
    code, out = checker(link_toolbox, argv(root, target, "lone", "--check"), tmp_path)
    assert code == 1
...
def test_plan_says_what_would_happen_and_stops(checker, tmp_path):
    ...
    root, target = toolbox(tmp_path), project(tmp_path)
    code, out = checker(link_toolbox, argv(root, target, "go", "--plan"), tmp_path)
    assert code == 0
